...
function playSound(type: 'crash' | 'success' | 'countdown' | 'final-countdown') {
  ...
  try {
    ...
    switch (type) {
      case 'crash':
        useAudio.getState().playExplosion(); // Use explosion sound for crash
        break;
      case 'success':
        useAudio.getState().playSuccess();
        break;
      case 'countdown':
        useAudio.getState().playCountdown();
        break;
      case 'final-countdown':
        useAudio.getState().playFinalCountdown();
        break;
    }
  } catch (error) {
  ...
  const startGame = () => {
    ...
    try {
      ...
      if ((window as any).audioPlayers && (window as any).audioPlayers['plane-music']) {
        ...
        // Fallback to original implementation
        useAudio.getState().startPlaneMusic();
      }
    } catch (error) {
  ...
  const endGame = (finalMultiplier: number) => {
    ...
    try {
      if ((window as any).audioPlayers && (window as any).audioPlayers['plane-music']) {
        ...
        // Fallback to original implementation
        useAudio.getState().stopPlaneMusic();
      }
    } catch (error) {
